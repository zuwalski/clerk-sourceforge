...
_ipt* cle_start2(task* parent, st_ptr config, st_ptr eventid, st_ptr userid, st_ptr user_roles, cle_pipe* response, void* responsedata) {
	_ipt* ipt;
	cle_instance inst;
	...
	if (cle_scan_validate(inst.t, &eventid, _scanner, &ctx)) {
		tk_drop_task(inst.t);
		return 0;
	}
