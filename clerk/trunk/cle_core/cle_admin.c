...
static int role_get_arguments(event_handler* hdl, struct _admin_role* args)
{
	args->eventmask = hdl->eventdata->eventid + sizeof(_allow_role_name);
	args->eventmask_length = hdl->eventdata->event_len - sizeof(_allow_role_name);
...
static void allow_next(event_handler* hdl)
{
	...
	if(role_get_arguments(hdl,&args))
	{
		cle_allow_role(hdl->instance_tk,args.eventmask,args.eventmask_length,args.role,args.role_length);
		cle_stream_end(hdl);
	}
	else
...
static void revoke_next(event_handler* hdl)
{
	...
	if(role_get_arguments(hdl,&args))
	{
		cle_revoke_role(hdl->instance_tk,args.eventmask,args.eventmask_length,args.role,args.role_length);
		cle_stream_end(hdl);
	}
	else
		cle_stream_fail(hdl,_illegal_argument,sizeof(_illegal_argument));
...
static void give_next(event_handler* hdl)
{
	...
	if(role_get_arguments(hdl,&args))
	{
		cle_give_role(hdl->instance_tk,args.eventmask,args.eventmask_length,args.role,args.role_length);
		cle_stream_end(hdl);
	}
	else
		cle_stream_fail(hdl,_illegal_argument,sizeof(_illegal_argument));
