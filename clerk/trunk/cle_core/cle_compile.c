...
static uint _cmp_var_assign(struct _cmp_state* cst, const uint state)
{
	...
	if(cst->c == '=')
	{
		...
		if(count == 0)
		{
			...
			_cmp_stack(cst,1);
			if(_cmp_expr(cst,0,ASGN_EXPR) != ';') err(__LINE__)
		...
		else
		{
			...
			do {
				term = _cmp_expr(cst,0,ASGN_EXPR);
			} while(term == ',');
			if(term != ';') err(__LINE__)
...
static void _cmp_for(struct _cmp_state* cst)
{
	...
	_cmp_nextc(cst);
	if(_cmp_expr(cst,0,FOR_EXPR) != 'd') err(__LINE__)
}
...
static int _cmp_expr(struct _cmp_state* cst, struct _skip_list* skips, uchar nest)
{
	...
	while(1)
	{
		...
		case '$':	// var (all states ok)
			if(state == ST_0 && (nest & (NEST_EXPR|FOR_EXPR|ASGN_EXPR)) == 0)
		...
		default:
			...
			else if(alpha(cst->c))
			{
				...
				case KW_VAR:
					chk_state(ST_0)
					if(nest & (NEST_EXPR|FOR_EXPR|ASGN_EXPR)) err(__LINE__)
					if(whitespace(cst->c)) _cmp_whitespace(cst);
