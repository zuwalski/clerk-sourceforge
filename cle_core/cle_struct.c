...
// little endian only here ... make no-op for big-endian cpu
#define ENDIAN(a) ((a >> 24) | \
				  ((a << 8) & 0x00FF0000) | \
				  ((a >> 8) & 0x0000FF00) | \
				   (a << 24))
...
__attribute__ ((always_inline)) static uint _st_cmp(cdat path, cdat ckey, uint max) {
	cdat path_from = path;
	uint a = 0;
	
	while (max >= 32) {
		if ((a = *(uint*)path ^ *(uint*)ckey)) {
			a = ENDIAN(a);
			...
			return a + (uint)(path - path_from) * 8;
		}
	...
	return (8 - a) + (uint)(path - path_from) * 8;
}

static uint _st_lookup_new(struct _st_lkup_res* rt)
{
	key* me   = rt->sub;
	...
	while(1)
	{
		uint d = _st_cmp(rt->path, ckey, STMIN(me->length - rt->diff, rt->length));
		
		rt->diff += d;
		rt->path += d >> 3;
		rt->length -= d & 0xfffffff8;
...
static uint _st_lookup2(struct _st_lkup_res* rt)
{
	key* me   = rt->sub;
	cdat ckey = KDATA(me) + (rt->diff >> 3);
	rt->d_sub = 0;
	...
	while(1)
	{
		uint max = STMIN(me->length - rt->diff, rt->length);
		cdat from = ckey;
		...
		rt->diff += max + (uint)(ckey - from) * 8;
		rt->length -= (uint)(ckey - from) * 8;
